...
\begin{abstract}
We present a reproducible, parameter-driven software workflow for optimizing approximate mutually unbiased basis configurations across dimensions, candidate basis counts, random seeds, numerical precisions, and hardware backends. The workflow uses a Lie-algebra parameterization of unitary bases,
$U_k = \exp(iH_k)$,
and records basis matrices, pairwise overlap tensors, optimization histories, metadata, and diagnostic summaries. The implementation was developed on consumer-level Apple Silicon hardware and designed for portable execution across CPU, Apple MPS, and NVIDIA CUDA/HPC environments. To support accelerator execution when native complex matrix exponentiation is unavailable, the workflow includes a Taylor-series matrix-exponential compatibility layer, while CPU structural experiments use native \texttt{torch.matrix\_exp} as the reference pathway.

As a case study, we apply the method to dimension six, where the existence of a complete set of seven MUBs remains unresolved. In multi-seed \texttt{complex128} experiments, the workflow recovers exact three-basis configurations, repeatedly identifies a structured four-basis basin consisting of three near-exact hub edges and a defective triangle, and finds only fully defective configurations for $n=5$ and $n=6$ under the stated tolerance. \texttt{complex64} experiments show that this qualitative transition is stable across precision, although near-exact classifications and basin selection can be precision-sensitive. HPC CPU/GPU campaigns and scaling benchmarks demonstrate that the same software stack runs across consumer and high-performance devices. The results should be interpreted as reproducible numerical evidence about the optimization landscape sampled by this workflow, not as a proof of existence or nonexistence of MUB configurations in dimension six.
\end{abstract}
...
In dimensions that are prime or powers of primes, complete sets of $d+1$ mutually unbiased bases are known to exist~\cite{WOOTTERS1989optimalStateMUB}. In composite dimensions~\cite{McNulty2026mutuallyunbiased}, and in particular in $d=6$, the existence of a complete set of seven MUBs remains a longstanding open problem~\cite{bengtsson2007MUBDandHadamard, horodecki2022FiveOpenProblems}. While numerous analytic and computational approaches have been explored~\cite{brierley2010mutuallyunbiasedbasesdimensions}, no definitive resolution is known. Historically, computational searches for MUBs in $d=6$ have frequently relied on anchored formulations that fix one or more bases (often to canonical Fourier matrices or specific families of Hadamard matrices) to restrict the parameter space. While computationally efficient, such constraints can skew the search space and obscure the broader geometry of the optimization landscape.
...
In the implementation, $H_k$ is obtained from an unconstrained trainable complex matrix $A_k \in \mathbb{C}^{d \times d}$ by Hermitian symmetrization. The anti-Hermitian component of $A_k$ is discarded by the symmetrization, so this implementation uses a redundant unconstrained representation of the Hermitian generator:
\begin{equation}
    H_k
    =
...
\end{equation}
Note that this parameterization uses $2d^2$ real variables (from the complex entries of $A_k$) to represent a space of Hermitian generators that only has dimension $d^2$. Although mathematically redundant, this unconstrained parameterization is numerically stable, avoids the need to maintain explicit parameter constraints, and is highly compatible with standard automatic differentiation and gradient-based optimization frameworks. The trainable variables are therefore the unconstrained matrices $A_1,\ldots,A_n$, while the matrices used in the loss are the unitary matrices generated by~\eqref{eq:lie-exp-param}. This separates the unitary constraint from the AMUB objective: the optimizer does not need to enforce unitarity through penalty terms or projections.
...
The multi-seed campaigns reported here use 100 independent seeds per value of $n$ for $n=3,\ldots,6$. This large-scale sweep provides a robust statistical characterization of the full nonconvex landscape and its basins.
...
The complex128 campaign used 100 independent seeds for each $n=3,4,5,6$. Table~\ref{tab:complex128-multiseed} summarizes the loss statistics and near-exact pair counts observed in the campaign.
...
For $n=5$, all 100 seeds produced fully defective configurations with no near-exact pairs. The best loss observed was approximately $0.359637$, while the median and maximum best losses were approximately $0.439033$ and $0.689537$, respectively. Most importantly, no near-exact pair was observed in any of the 100 runs.
...
\begin{table*}[t]
  \caption{Multi-seed complex128 campaign for $n=3,\ldots,6$. The near-exact counts list the min/median/max number of near-exact pairs observed across 100 random seeds, using tolerance $10^{-6}$.}
...
We repeated the multi-seed campaign for $n=3,4,5,6$ using complex64 arithmetic with 100 independent random seeds. Table~\ref{tab:precision-comparison} compares the complex128 and complex64 summaries. As described in Section~\ref{sec:experimental-methodology}, these structural precision comparisons use the CPU matrix-exponential pathway rather than the Taylor GPU backend.
...
\begin{table*}[t]
  \caption{Precision comparison for multi-seed AMUB campaigns in dimension six. Near-exact min/med/max use the primary tolerance $10^{-6}$ across 100 independent seeds.}
...
\begin{table*}[ht]
  \caption{Multi-dimensional hardware benchmark and iteration speed comparison (in seconds) per $1000$ optimization steps ($n=6$ bases) across dimensions $d=6, 12, 24, 48, 96$ on the JANUS HPC cluster (AMD EPYC 7352 CPU vs. NVIDIA A100 GPU). Order-$20$ Taylor expansion is utilized for the GPU backend.}
  \label{tab:hardware-benchmarks-janus}
  \begin{tabular}{ccccc}
    \toprule
    Dimension $d$ & CPU \texttt{c128} (s) & CPU \texttt{c64} (s) & A100 GPU (s) & Relative GPU Speed \\
    \midrule
  ...
  \end{tabular}
\end{table*}
\noindent Relative GPU speed is computed as CPU complex128 time divided by GPU complex128 time.
...
\begin{figure}[ht]
  ...
  \caption{Best AMUB loss summary in $d=6$. This plot shows the median best loss with min--max range across 100 seeds. The AMUB loss increases with $n$.}
...
\begin{figure}[t]
  ...
  \caption{Near-exact MUB pair counts in $d=6$. This plot shows the median number of near-exact MUB pairs with corresponding ranges across 100 seeds. Near-exact pairs disappear for $n \ge 5$ under the primary tolerance, coinciding with a transition to fully defective observed configurations.}
